const TournamentErrorsKYC = Object.freeze({
  NO_TOURNAMENT: 1,
  NOT_CLEAR: 2,
  NAMES_ALREADY_TAKEN: 3,
});
try {
  module.exports = {
    TournamentErrorsKYC: TournamentErrorsKYC,
  };
} catch (e) {}
(function () {
...
function render(tmpl_name, tmpl_data) {
  if (!render.tmpl_cache) render.tmpl_cache = {};
  if (tmpl_name.startsWith('/')) tmpl_name = tmpl_name.slice(1);
  if (!render.tmpl_cache[tmpl_name]) {
    var tmpl_string;
    $.ajax({
      url: `${Config.asset_location}views/${tmpl_name}.html`,
      method: 'GET',
      async: false,
    ...
    if (!tmpl_string) {
      $.ajax({
        url: `${Config.asset_location_backup}views/${tmpl_name}.html`,
        method: 'GET',
        async: false,
        cache: SM.settings.template_caching,
        success: (data) => (tmpl_string = data),
...
function scPayment(to, amount, currency, memo) {
  let sc_url = `https://steemconnect.com/sign/transfer?to=${to}&amount=${parseFloat(
    amount,
  ).toFixed(3)}%20${currency}&memo=${encodeURIComponent(memo)}`;
  popup_center(sc_url, `${currency} Payment`, 630, 660);
}
function hiveSignerPayment(to, amount, currency, memo) {
  let sc_url = `https://hivesigner.com/sign/transfer?to=${to}&amount=${parseFloat(
    amount,
  ).toFixed(3)}%20${currency}&memo=${encodeURIComponent(memo)}`;
  popup_center(sc_url, `${currency} Payment`, 630, 660);
}
function vesselPayment(to, amount, currency, memo) {
  ...
  window.location =
    'steem://sign/tx/' +
    btoa(transaction) +
    '#eyJhbW91bnQiOnsicHJvbXB0IjpmYWxzZSwidHlwZSI6ImFzc2V0IiwibGFiZWwiOiJEb25hdGlvbiJ9LCJtZW1vIjp7InByb21wdCI6ZmFsc2UsInR5cGUiOiJ0ZXh0IiwibGFiZWwiOiJNZXNzYWdlIChPcHRpb25hbCkifSwidG8iOnsicHJvbXB0IjpmYWxzZSwidHlwZSI6InRleHQifX0=';
}
...
function sortCards(a, b) {
  ...
  var a_index =
    colors.indexOf(a.color || a.card_details.color) * 1e4 +
    (a.rarity || a.card_details.rarity) * 1e3 +
    (a.id || a.card_details.id);
  var b_index =
    colors.indexOf(b.color || b.card_details.color) * 1e4 +
    (b.rarity || b.card_details.rarity) * 1e3 +
    (b.id || a.card_details.id);
...
function getHeadBlock(callback) {
  steem.api.getDynamicGlobalPropertiesAsync().then((result) => {
    if (callback) callback(result.head_block_number, result.time);
  });
}
async function getHeadBlockAsync() {
  return await steem.api
    .getDynamicGlobalPropertiesAsync()
    .then((result) => result.head_block_number)
    .catch(async (err) => SM.GetCurrentBlockNum());
}
...
function getLevel(xp, rarity) {
  for (var i = 0; i < SM.settings.xp_levels[rarity - 1].length; i++) {
    if (xp < SM.settings.xp_levels[rarity - 1][i]) return i + 1;
  }
...
function constructTournamentAllowedEditionsText(editions) {
  ...
  if (allowed_editions.length === 0) {
    allowed_editions = [0, 1, 2, 3, 4, 5, 7];
  }
  ...
  if (
    ...
    editionNames.length === SM.settings.num_editions
  ) {
...
function sendGuildChatNotification(title, message, guild_id) {
  if (localStorage.getItem('sl:guild_chat_notif_disabled')) return;
  desktopNotification(
    title,
    'https://d36mxiodymuqjm.cloudfront.net/website/nav/icon_nav_guilds_active@2x.png',
    message,
    () => SM.Guilds.ShowGuild(guild_id, 'tavern'),
  );
}
function sendNotification(title, icon, message, onclick) {
  if (!SM.Player || !SM.Player.settings.notifications_enabled) return;
...
function xss(text) {
  while (text.match(/onload/gi)) text = text.replace(/onload/gi, '');
  while (text.match(/onerror/gi)) text = text.replace(/onerror/gi, '');
  while (text.match(/javascript/gi)) text = text.replace(/javascript/gi, '');
  while (text.match(/&#/gi)) text = text.replace(/&#/gi, '');
  while (text.match(/<\/?script>/gi)) text = text.replace(/<\/?script>/gi, '');
  return text;
}
...
function toUSD(payment) {
  return (
    parseFloat(payment) *
    SM.settings[getCurrency(payment).toLowerCase() + '_price']
  ).toFixed(2);
}
function calculateDEC(card, details) {
  let total_dec = calculateCP(card);
  return Math.floor(total_dec);
}
function calculateCP(card, details, base_only) {
  if (!details) {
    details = SM.GetCardDetails(card.card_detail_id);
  }
  ...
  let burn_rate =
    card.edition == 4 || details.tier >= 4
      ? SM.settings.dec.untamed_burn_rate[details.rarity - 1]
      : SM.settings.dec.burn_rate[details.rarity - 1];
  if (alpha_xp && !isBaseOnly) {
    let alpha_bcx_xp =
      SM.settings[card.gold ? 'gold_xp' : 'alpha_xp'][details.rarity - 1];
    alpha_bcx = Math.max(
      card.gold ? alpha_xp / alpha_bcx_xp : alpha_xp / alpha_bcx_xp,
      1,
    );
    alpha_dec = burn_rate * alpha_bcx * SM.settings.dec.alpha_burn_bonus;
    if (card.gold) alpha_dec *= SM.settings.dec.gold_burn_bonus;
  }
  const xp_property =
    card.edition == 0 || (card.edition == 2 && details.id < 100)
      ...
      ? 'beta_gold_xp'
      : 'beta_xp';
  const bcx_xp = SM.settings[xp_property][details.rarity - 1];
  ...
  if (card.gold) {
    const gold_burn_bonus_prop =
      details.tier >= 7 ? 'gold_burn_bonus_2' : 'gold_burn_bonus';
    dec *= SM.settings.dec[gold_burn_bonus_prop];
  }
  if (card.edition == 0) dec *= SM.settings.dec.alpha_burn_bonus;
  if (card.edition == 2 && details.tier == null)
    dec *= SM.settings.dec.promo_burn_bonus;
  let total_dec = dec + alpha_dec;
  if (!isBaseOnly && card.xp >= getMaxXp(details, card.edition, card.gold))
...
function calculateECR(capture_rate, last_reward_time) {
  return Math.min(
    (isNaN(parseInt(capture_rate)) ? 1e4 : capture_rate) +
      ((Date.now() - new Date(last_reward_time)) / 3e3) *
        SM.settings.dec.ecr_regen_rate,
    1e4,
  );
}
function isCardLocked(card) {
  return (
    card.lock_days > 0 &&
    (!card.unlock_date || new Date(card.unlock_date) > new Date())
  );
}
...
function getBalance(symbol) {
  if (!SM.Player || !SM.Player.balances) return 0;
  var record = SM.Player.balances.find((b) => b.token == symbol);
  return record ? record.balance : 0;
}
_sponsors = {
  'bitcoin-com':
    'https://exchange.bitcoin.com/?utm_source=ingame&utm_medium=banner&utm_campaign=exchange_promo&utm_term=splinterlands',
};
...
function countUp(element, from, to, decimals) {
  ...
  var decimals_to = decimals
    ? decimals
    : to >= 1e3
    ? 0
    : Math.min(countDecimals(to), 3);
  if (amount < 1) {
    element.text(addCommas(truncate(to, decimals_to)));
    return;
  }
  step = amount / 150;
  update();
  function update() {
    setTimeout(() => {
      from += step;
      element.text(addCommas(truncate(from, decimals)));
      if (from < to) update();
      else element.text(addCommas(truncate(to, decimals_to)));
    }, interval);
  }
}
...
function getSkins(card_detail_id) {
  if (!SM.Player || !SM.Player.skins) return [];
...
function getDecCost(cost) {
  return SM.Player && SM.Player.guild && SM.Player.guild.quest_lodge_level
    ? +(
        cost *
        (1 -
          SM.settings.guilds.shop_discount_pct[
            SM.Player.guild.quest_lodge_level - 1
          ] /
            100)
      ).toFixed(1)
    : cost;
...
function getStarterCard(card_details) {
  let starter_edition = parseInt(
    card_details.editions
      .split(',')
      .find((e) => SM.settings.starter_editions.includes(parseInt(e))),
  );
  if (!starter_edition || card_details.rarity > 2) return null;
  return {
...
function cardsToLevel(card_detail_id, edition, gold, level) {
  ...
  if (edition == 4 || details.tier >= 4) {
    let rates = gold
      ? SM.settings.combine_rates_gold[details.rarity - 1]
      : SM.settings.combine_rates[details.rarity - 1];
    ...
    let bcx =
      SM.settings[
        edition == 0 || (edition == 2 && card_detail_id < 100)
          ? gold
            ? 'gold_xp'
            : 'alpha_xp'
          : gold
    ...
    let gold_na = [3, 2, 2, 1];
    return gold
      ? level <= gold_na[details.rarity - 1]
        ? 'N/A'
        : Math.ceil(xp_levels[level - 2] / bcx)
      : level == 1
      ? 1
      : Math.ceil(xp_levels[level - 2] / bcx) + 1;
  }
}
function getMaxXp(details, edition, gold) {
  ...
  if (edition == 4 || tier >= 4) {
    let rates = gold
      ? SM.settings.combine_rates_gold[rarity - 1]
      : SM.settings.combine_rates[rarity - 1];
    return rates[rates.length - 1];
  } else
    return SM.settings.xp_levels[rarity - 1][
      SM.settings.xp_levels[rarity - 1].length - 1
    ];
}
...
async function getTokenPrice(token) {
  return fetch('https://prices.splinterlands.com/prices')
    .then((r) => r.json())
    .then((r) => (!token ? r : r[token]));
}
window.abilities = [
  {
    name: 'Affliction',
  ...
  },
  {
    ...
    effect_name: 'Enraged',
    effect_desc:
      'Enraged monsters get increased speed and attack damage when not at full health.',
  },
  {
    name: 'Flying',
...
window.abilities = window.abilities.sort((a, b) =>
...
function getSupportedCurrency(symbol) {
  ...
  return SM.settings.supported_currencies.find(
    (c) => c.currency == symbol.toUpperCase(),
  );
}
function and() {
...
var _init_url_search_params = new URLSearchParams(
  _urlHash.substring(_urlHash.indexOf('?')),
);
...
function removeTxPrefix(tx_name) {
  return tx_name.replace(
    SM.settings.test_mode ? `${SM.settings.prefix}sm_` : 'sm_',
    '',
  );
}
function fsm(schema) {
  ...
  return {
    ...
    transition(event, state, prevState) {
      ...
      if (!this.canTransition(event, curState)) return;
      ...
      callAll(
        [this.onTransition].concat(nextState.onEnter || []),
        event,
        state,
        prevState,
      );
...
SM = {
  ...
  _rpc_index: 0,
  AuthHeader: function (request, type = 'jquery') {
    switch (type) {
      case 'jquery':
        return request.setRequestHeader(
          'Authorization',
          `Bearer: ${SM.Player ? SM.Player.jwt_token : null}`,
        );
      case 'fetch':
        return {
          Authorization: `Bearer ${SM.Player ? SM.Player.jwt_token : null}`,
        };
    }
  },
  Api: function (url, data, callback, always) {
    ...
    jQuery
      .ajax({
        beforeSend: (request) => SM.AuthHeader(request),
        type: 'GET',
        dataType: 'json',
      ...
      .fail((e) => {
        ...
        if ([401, 403].includes(e.status)) SM.Logout();
      })
      .always(function () {
        if (always) always();
      });
  },
  ApiPromise: function (url, data) {
    if (data == null || data == undefined) data = {};
    data.v = new Date().getTime();
    if (SM.Player) {
      data.token = SM.Player.token;
  ...
  ECApi: function (url, data, callback, always) {
    ...
    jQuery
      .ajax({
        ...
        type: 'GET',
        dataType: 'json',
        url: Config.external_chain_api_url + url,
        data: data,
        success: function (response) {
          if (callback != null && callback != undefined) callback(response);
        },
  ...
  async Put(url, data) {
    ...
    return fetch(url, {
      method: 'PUT',
      headers: SM.AuthHeader(null, 'fetch'),
  ...
  SetTranslator: function setTranslator(polyglot, defaultLocale) {
    ...
    SM.Translator = {
      locale(l) {
        ...
        SM.ShowLoading();
        $.get(
          `${Config.asset_location}locale/${l}.json?v=${asset_version}`,
          (res) => {
  ...
  TranslateIndex: function translateIndex() {
    ...
    elements.forEach(function (el) {
      ...
      if (translation == el.dataset.translate) return;
      if (el.text) {
        el.text = translation;
  ...
  ShowHomeView: function (view, data, url_params) {
    ...
    if (SMBPortrait.Team1 != null) SMBPortrait.Team1.CleanUp();
    if (SMBPortrait.Team2 != null) SMBPortrait.Team2.CleanUp();
    if (window.innerWidth <= 990) {
      var burger = $('.navbar-toggle');
      if (!burger.hasClass('collapsed')) burger.click();
    }
    var url =
      view == 'about'
        ? '/'
        : '?p=' + view + (url_params ? '&' + $.param(url_params) : '');
    ...
    if (
      current_tournament &&
      current_tournament.format !== 'swiss' &&
      current_tournament.matchup &&
      view != 'tournament' &&
      view != 'create_team2' &&
      view != 'battle' &&
      (!SM._noties.tournament_return || SM._noties.tournament_return.closed)
    )
      SM.ShowNoty(
        'tournament_return',
        'tournament_return',
  ...
  _loading: $(
    '<div class="modal-backdrop fade in loading-backdrop"><img src="https://d36mxiodymuqjm.cloudfront.net/website/loading-spinner_500.gif" class="loading" /></div>',
  ),
  ...
  ShowPackOpenVideo: function (edition) {
    var video = $(
      '<div id="divVideo" style="position:absolute; top: 0; z-index: 10000; width: 100vw;">' +
        '<video style="width: 100%;" onended="SM.PackOpenVideoEnded();" onclick="this.play()" autoplay>' +
        '<source src="https://d36mxiodymuqjm.cloudfront.net/website/vfx/pack-opening_' +
        (edition === 7 ? '7_hires' : edition) +
        '.mp4" type="video/mp4">' +
        '</video></div>',
    );
    video.appendTo('body');
    switch (edition) {
      case 6:
        SMAudio.PlaySound(
          `pack_open_${edition}`,
          'https://d36mxiodymuqjm.cloudfront.net/website/sfx/SiteUI_GladiusCase_PackOpenAnimation.mp3',
        );
        break;
      case 7:
        SMAudio.PlaySound(
          `pack_open_${edition}`,
          'https://d36mxiodymuqjm.cloudfront.net/website/sfx/SiteUI_ChaosLegion_PackOpenAnimation.mp3',
        );
        break;
      default:
        SMAudio.PlaySound(
          `pack_open_${edition}`,
          'https://d36mxiodymuqjm.cloudfront.net/website/sfx/SiteUI_OpenPack_PackOpenAnimation.mp3',
        );
        break;
    }
  },
  RenderDialog: function (dialog, data, onClose, no_backdrop) {
    if (dialog.startsWith('/')) dialog = dialog.slice(1);
    $('#dialog_container').html(
      render('dialogs/' + dialog, {
        data: data,
      }),
    );
    ...
    if (onClose) SM._activeDialog.on('hidden.bs.modal', onClose);
  },
  HideDialog: function (cancelEvents, onHidden) {
    ...
    if (cancelEvents) SM._activeDialog.off('hidden.bs.modal');
    if (onHidden) {
  ...
  ShowUrlPage(url) {
    if (getURLParameter('status') == 'paypal_complete') SM.PaypalComplete();
    var parts = JSON.parse(
      '{"' +
        xss(decodeURI(url))
          .replace(/"/g, '\\"')
          .replace(/&/g, '","')
    ...
    switch (parts.p) {
      ...
      case 'card_details':
        if (SM.Player)
          SM.LoadCards(() =>
            SM.LoadCollection(SM.Player.name, true, () =>
              SM.ShowCardDetails(
                parts.id,
                parts.gold == 'true',
                parseInt(parts.edition),
                parts.tab,
              ),
            ),
          );
      ...
      case 'battle_history':
        ...
        break;
      case 'battle':
        if (parts.id) SM.LoadBattle(parts.id);
        break;
      case 'tournaments':
        SM.ShowTournaments();
        break;
      ...
      case 'verify_email':
        SM.VerifyEmail(
          decodeURIComponent(parts.e),
          parts.ts,
          decodeURIComponent(parts.sig),
        );
        break;
  ...
  ShowLogin: function (callback) {
    if (SM.Player) {
      SM.ShowBattleHistory();
    } else SM.ShowDialog('login', null, null, callback);
  },
  Logout: function () {
  ...
  Init: function () {
    ...
    SM.LoadSettings(function () {
      ...
      SM.LoadCards();
      SM.LoadItems();
      if (getURLParameter('verify_raffle')) SM.ShowHomeView('raffle_verifier');
      if (getURLParameter('verify_rewards')) SM.ShowHomeView('reward_verifier');
  ...
  OnLogin: async function (is_new) {
    ...
    let bio_name = SM.Player.display_name || SM.Player.name;
    if (SM.settings.maintenance_mode && SM.Player && SM.Player.is_team) {
      bio_name += ' (M)';
    }
    $('#log_in_text .bio__name__display').text(bio_name);
    ...
    if (!SM.settings.maintenance_mode || SM.Player.is_team) {
      ...
      SM.Player.messages
        .filter((m) =>
          ['payment', 'brawl_fray_status', 'brawl_only_guild'].includes(m.type),
        )
        .forEach((m) => {
          ...
          if (m.type == 'payment') {
            ...
            m.type == 'brawl_fray_status' &&
            SM.Player.guild &&
            SM.Player.guild.id === data.guild_id &&
            SM.Player.guild.tournament_id === data.tournament_id
          ) {
            if (data.type === 'cancel') {
            ...
            m.type == 'brawl_only_guild' &&
            SM.Player.guild &&
            SM.Player.guild.id === data.guild_id &&
            SM.settings.brawl_cycle.id === data.brawl_cycle_id
          ) {
            SM.ShowNoty(
              `${data.block_num}-${data.guild_id}`,
              'brawl_only_guild',
              data,
              false,
            );
      ...
      let lottery_result_msg = SM.Player.messages.find(
        (m) => m.type == 'lottery_result',
      );
      if (
        !suppress_brawl_in_progress &&
        SM.Player.guild &&
        SM.Player.guild.tournament_id &&
        SM.Player.guild.tournament_status === 0 &&
        isNaN(SM.Player.guild.my_fray_index)
      ) {
        let frays_open =
          SM.settings.frays[SM.Player.guild.tournament_data.challenge_level]
            .length - SM.Player.guild.frays_filled;
        if (frays_open > 0) {
          SM.ShowNoty(
            SM.Player.guild.tournament_id + '-in-progress',
            'brawl_in_progress',
            {
      ...
      if (
        SM.Player.outstanding_match &&
        !SM.Player.outstanding_match.is_critical
      ) {
        if (
          SM.Player.outstanding_match.tournament_format &&
          SM.Player.outstanding_match.tournament_format === 'swiss'
        ) {
          SM.ShowNoty(
            SM.Player.outstanding_match.tournament_id + '-in-progress',
            'swiss_in_progress',
            {
        ...
        if (
          SM.Player.outstanding_match.brawl_id &&
          SM.Player.guild &&
          !suppress_brawl_in_progress
        ) {
          SM.ShowNoty(
            SM.Player.outstanding_match.brawl_id + '-in-progress',
            'brawl_in_progress',
            {
      ...
      if (
        ...
        SM.ShowDialog(
          'shop/lottery_results',
          tryParse(lottery_result_msg.data),
        );
      } else if (!SM.ShowAnnouncement(is_new)) {
        let gifts = SM.Player.messages
          .filter((m) => m.type == 'received_gifts')
          .map((g) => {
            var data = tryParse(g.data);
            if (data) data.msg_id = g.id;
            return data;
          })
          .filter((g) => g != null);
        if (gifts.length > 0) SM.ShowDialog('gifts2', gifts);
        SM.ShowMysteryRewards(
          SM.Player.messages.filter((m) => m.type == 'mystery_reward'),
        );
        SM.Player.messages
          .filter((m) => m.type == 'guild_update')
          .forEach((m) => {
            let data = tryParse(m.data);
            SM.MarkAsRead(m.id);
    ...
    SM.CheckUnclaimedAirdrops();
    if (SM.Player.is_admin || SM.Player.is_cs_admin) $('#nav_admin').show();
    ...
    gtag('event', 'login', {
      ...
      page_path:
        localStorage.getItem('splinterlands:page_path') ||
        `/${window.location.search}`,
    });
  },
  UpgradeAccount: function (name, key) {
    SM.BroadcastCustomJson(
      'sm_upgrade_account',
      'Upgrade Account',
      {
        account_name: name,
  ...
  LoadBalances: function (callback, always) {
    if (callback) callback(SM.Player.balances);
    if (always) always();
  },
  OnLoadBalances: function (balances) {
    if (SM.Player) {
      ...
      var ecr = Math.round(
        calculateECR(SM.Player.capture_rate, SM.Player.last_reward_time),
      );
      $('.dec-right .dec-container .ecr-bar').css(
        'width',
        Math.round((ecr / 1e4) * 100) + 'px',
      );
      $('.dec-left .dec-container .ecr-bar').css(
        'width',
        Math.round((ecr / 1e4) * 100) + 'px',
      );
      $('.dec-container .dec-options').remove();
      $('.dec-container').append(
        SM.ShowComponent('dec_options', {
  ...
  GetBalance: function (token, player) {
    player = player || SM.Player;
    if (!player || !player.balances) return 0;
    var balance = player.balances.find((b) => b.token == token);
    return balance ? parseFloat(balance.balance) : 0;
  },
  CheckUnclaimedAirdrops() {
    SM.Api(
      ...
      (airdrops) => {
        ...
        let airdrop = airdrops.find((a) => !a.claim_date);
        if (!airdrop) {
          return;
        }
        for (let i = 0; i < airdrop.cards.length; i++) {
          const card = airdrop.cards[i];
          if (
            card.claim_date &&
            new Date(card.claim_date).getTime() - Date.now() > 0
          ) {
            return;
          }
        }
        SM.ShowNoty('airdrop-unclaimed', 'airdrop_unclaimed', airdrop, false);
      },
    );
  },
  ShowUnrevealedRewards: function (result) {
    ...
    if (!result.rewards || result.rewards.length == 0) return;
    var n = new Noty({
      type: 'reveal_rewards',
      data: {
  ...
  RevealRewards: function (result, type) {
    ...
    let sg_promo = cards.some(
      (c) => c.card_detail_id >= 114 && c.card_detail_id <= 117,
    );
  ...
  Login: async function (username, key, callback) {
    let using_master_key = false;
    if (key && key.startsWith('STM')) {
      if (callback)
        callback({
          success: false,
          error:
            'This appears to be a public key. You must use your private posting key to log in.',
        });
      return;
    }
    ...
    if (!key) key = localStorage.getItem('key');
    SM._use_keychain = !key;
    username =
      username && username.length > 0 ? username.toLowerCase().trim() : '';
    if (username && username.startsWith('@')) username = username.substr(1);
    if (
      (!window.hive_keychain && (!username || !key)) ||
      (window.hive_keychain && !username)
    ) {
      if (callback)
        callback({
          success: false,
    ...
    if (!SM._use_keychain) {
      try {
        ...
        if (callback)
          callback({
            success: false,
            error:
              'Invalid password or private posting key for account @' +
              username,
          });
        return;
      }
      ...
      params.sig = await new Promise((resolve) =>
    ...
    SM.Api('/players/login', params, function (response) {
      ...
      if (using_master_key) {
        if (callback)
          callback({
            success: false,
            error: 'error-non-posting-key',
          });
        return;
      }
      localStorage.setItem('username', username);
      if (!SM._use_keychain) localStorage.setItem('key', key);
      SM.Player = response;
      if (callback)
        callback({
          success: true,
  ...
  LoginWithWax: function (params) {
    ...
    const w = window.open(
      `${Config.api_url}/auth/wax/waa?${querystring}`,
      '_blank',
  ...
  EmailLogin: async function (email, password, is_new) {
    ...
    return await new Promise((resolve) => {
      SM.Login(response.username, response.posting_key, (r) => {
        if (r && r.success) SM.OnLogin(is_new);
        resolve(r);
      });
    });
  },
  CreateAccountEOS: async function (email, subscribe, is_test) {
    let account = await WalletUtils.getScatterIdentity(
      WalletUtils.config.scatter.eos_network,
      'eos',
    );
    let params = {
      login_type: 'eos',
    ...
    if (response && !response.error) {
      ...
      if (login_response && login_response.success) SM.HideDialog();
  ...
  EOSLogin: async function (is_new) {
    try {
      let params = await WalletUtils.scatterAuth(
        WalletUtils.config.scatter.eos_network,
        'eos',
      );
  ...
  ArkaneAuth: async function () {
    await loadScriptAsync(
      'https://unpkg.com/@arkane-network/arkane-connect@1.19.0/dist/connect.js',
    );
    const arkaneConnect = new ArkaneConnect(
      SM.settings.arkane ? SM.settings.arkane.client_id : 'Splinterlands',
      {
    ...
    return new Promise((resolve) => {
      authResult
        .authenticated((instance) => resolve([arkaneConnect, instance]))
        .notAuthenticated((instance) =>
          resolve([
            arkaneConnect,
            {
  ...
  CreateAccountArkane: async function (subscribe, is_test) {
    ...
    if (!authResponse.authenticated) return authResponse;
    ...
    if (response && !response.error) {
      let login_response = await SM.ArkaneLogin(true, [
        arkaneConnect,
        authResponse,
      ]);
      if (login_response && login_response.success) SM.HideDialog();
  ...
  ArkaneLogin: async function (is_new, arkaneAuth) {
    ...
    const loginResponse = await fetch(
      `${Config.external_chain_api_url}/players/login_arkane?${querystring}`,
    );
  ...
  ShowAnnouncement: function (is_new) {
    ...
    if (
      SM.settings.promotions &&
      SM.settings.promotions.includes('brave') &&
      !localStorage.getItem('sm-brave_shown')
    ) {
    ...
    if (
      SM.settings.promotions &&
      SM.settings.promotions.includes('waka_promo') &&
      !localStorage.getItem('sm-waka_promo_shown')
    ) {
      localStorage.setItem('sm-waka_promo_shown', true);
      SM.ShowDialog('misc/promotions/waka_promo');
      return true;
    }
    let last_shown = localStorage.getItem('sm-daily_update_shown');
    if (
      !last_shown ||
      Date.now() - new Date(last_shown).getTime() > 8 * 60 * 60 * 1e3
    ) {
      localStorage.setItem('sm-daily_update_shown', new Date().toISOString());
      if (
        SM.settings.promotions &&
        SM.settings.promotions.includes('land_presale')
      )
        SM.ShowDialog('shop/land_presale');
      else SM.ShowDailyUpdate();
      return true;
    }
    return false;
  },
  ...
  ShowMarket: function (tab) {
    ...
    SM.ShowHomeView(
      ['land', 'other'].includes(tab) ? 'market_coming_soon' : 'market',
      {
        tab: tab || 'cards',
      },
      {
  ...
  ShowFAQ: function () {
    ...
    if (SM.settings.zendesk_enabled == 1)
      window.open('https://support.splinterlands.com', '_blank').focus();
    else
      SM.Api('/players/faq/' + localStorage.getItem('locale'), null, (r) =>
        SM.ShowHomeView('faq', r),
      );
  },
  ...
  LoadMarketData: function (type, level, callback, always) {
    ...
    SM.Api(
      ...
      function (response) {
          ...
          return Object.assign(
            {
              ...
              armor:
                card.type === 'Monster'
                  ? card.stats.armor[r.level - 1]
                  : card.stats.armor,
              attack:
                card.type === 'Monster'
                  ? card.stats.attack[r.level - 1]
                  : card.stats.attack,
              health:
                card.type === 'Monster'
                  ? card.stats.health[r.level - 1]
                  : card.stats.health,
              magic:
                card.type === 'Monster'
                  ? card.stats.magic[r.level - 1]
                  : card.stats.magic,
              ranged:
                card.type === 'Monster'
                  ? card.stats.ranged[r.level - 1]
                  : card.stats.ranged,
              speed:
                card.type === 'Monster'
                  ? card.stats.speed[r.level - 1]
                  : card.stats.speed,
              circulation: SM.GetCardDistribution(
                r.card_detail_id,
                r.gold,
                r.edition,
              ).num_cards,
        ...
        if (!level) {
          if (type == 'rentals') SM._rental_items_grouped = data;
          else SM._market_items_grouped = data;
        }
  ...
  ShowShop: async function (tab) {
    ...
    if (
      ...
      (!SM.Player ||
        !SM.Player.guild ||
        SM.Player.guild.buildings.guild_shop.level < 1)
    ) {
      tab = 'packs';
    }
    SM.ShowHomeView(
  ...
  ShowOpenPacks: async function (pending) {
  ...
  ShowItemInvetory: function (player, filters) {
    if (player && player.length > 17) player = null;
    if (!player && SM.Player) player = SM.Player.name;
  ...
  LoadCollection: async function (name, reload, callback, always) {
    if (
      name &&
      (!SM._collectionLoaded || reload || SM.Player.collection_dirty)
    ) {
      ...
      SM.Api(
        ...
        function (response) {
          ...
          for (var i = 0; i < response.cards.length; i++) {
            ...
            var collection_card = collection.find(
              (c) => c.id == card.card_detail_id,
            );
            if (collection_card)
              collection_card.owned.push(
                Object.assign(card, SM.GetCardLevelInfo(card)),
              );
          }
            ...
            if (SM.Player && SM.Player.name == name && c.owned.length == 0) {
              ...
              if (starter_card)
                c.owned.push(
                  Object.assign(
                    starter_card,
                    SM.GetCardLevelInfo(starter_card),
                  ),
                );
            }
          });
    ...
    } else if (callback) callback(SM.cards);
  },
      ...
      details.editions.split(',').forEach((edition) => {
        result.push({
          ...
          owned: details.owned
            ? details.owned
                .filter(
                  (o) => o.gold == false && o.edition == parseInt(edition),
                )
                .slice()
            : [],
        });
        result.push({
          ...
          owned: details.owned
            ? details.owned
                .filter((o) => o.gold == true && o.edition == parseInt(edition))
                .slice()
            : [],
        });
      });
  ...
  LoadSkins: async function () {
    ...
    SM._skin_sets = SM._skins
      .reduce((t, v) => {
        ...
        else if (v.remaining <= 0) set.available = false;
        return t;
      }, [])
  ...
  GetCardStats: function (card) {
    var level = Math.min(
      card.max_level || 100,
      card.level || SM.GetCardLevelInfo(card).level,
    );
    var details = SM.GetCardDetails(card.card_detail_id || card.id);
  ...
  GetCardBCX: function (card) {
    ...
    return Math.max(
      card.gold ? card.xp / bcx_xp : (card.xp + bcx_xp) / bcx_xp,
      1,
    );
  },
  CalcCardValue: function (card) {
    ...
    var price_per_bcx = 0;
    var market_item = SM._market_items_grouped.find(
      (i) =>
        i.card_detail_id == card.card_detail_id &&
        i.edition == card.edition &&
        i.gold == card.gold,
    );
    if (market_item) price_per_bcx = market_item.low_price_bcx;
    return price_per_bcx * bcx;
  },
  CalcCollectionValue: function (collection) {
    var total_value = 0;
    collection.forEach((details) => {
      if (!details.owned || details.owned.length == 0) return;
  ...
  ShowCardDetails: function (detail_id, gold, edition, tab, refresh, external) {
    ...
    if (
      ['market', 'rentals'].includes(tab) &&
      SM.settings.soulbound_editions.includes(edition)
    ) {
      tab = 'cards';
    }
    SM.ShowHomeView(
  ...
  ShowCardInfo: function (uid, tab) {
    SM.LoadCardsByUid(uid, (card_data) =>
      SM.ShowHomeView(
        'card_info',
        Object.assign(
          {
  ...
  StartPurchase: function (
    ...
    return new Promise((resolve, reject) => {
      if (isNaN(quantity) || quantity <= 0) reject('Invalid quantity');
      ...
      if (['HIVE', 'HBD', 'DEC', 'CREDITS'].includes(currency)) {
        ...
        SM.ApiAsync('/purchases/start', {
          ...
          if (res.error) {
            reject(
              `There was an error initiating the purchase transaction: ${res.error}`,
            );
          }
          resolve(res);
        });
  ...
  RedeemPromoCode: function (purchase_id, code, callback) {
    SM.Api(
      ...
      function (response) {
        if (response.error)
          alert(
            'There was an error redeeming the promo code: ' +
  ...
  GetUnrevealedPacks: function () {
    var unrevealed = localStorage.getItem('sl_unrevealed_pack_openings');
    return unrevealed ? unrevealed.split(',') : [];
  },
  _pack_data: null,
    ...
    SM.BroadcastCustomJson(
      ...
      (result) => {
        if (!result || result.error) {
          ...
          var pack_balance = SM.Player.balances.find(
            (b) => b.token === getEdition(edition).toUpperCase(),
          );
          if (pack_balance) pack_balance.balance -= 1;
          SM.ShowOpenPacks(true);
          return;
        }
        if (result && result.trx_info && result.trx_info.result) {
          var data = tryParse(result.trx_info.result);
          if (data && data.cards) {
            var unrevealed = SM.GetUnrevealedPacks();
            if (
              unrevealed &&
              unrevealed.length > 0 &&
              unrevealed.includes(result.trx_info.id)
            ) {
              unrevealed.splice(unrevealed.indexOf(result.trx_info.id), 1);
              localStorage.setItem('sl_unrevealed_pack_openings', unrevealed);
            }
            SM._pack_data = {
              cards: data.cards,
              edition: edition,
  ...
  PackOpenVideoEnded: function (error) {
    ...
    if (!error && SM._pack_data) {
      SM.HideLoading();
      SM.ShowDialog('open_pack', SM._pack_data, null, null, true);
    } else if (!error) SM.ShowLoading();
  },
  OpenAllPacks: function (edition, qty) {
    const alpha_edition = 0;
    const gladius_edition = 6;
    let pack_str = [
      'Alpha Packs',
      'Beta Packs',
      'Essence Orbs',
    ...
    if (
      !confirm(
        `This will open ${qty || 'all'} of your unopened ${
          pack_str[edition]
        } at once. Are you sure you want to continue?`,
      )
    )
      return;
    let gold_potions = SM.GetBalance('GOLD');
    let legendary_potions = SM.GetBalance('LEGENDARY');
    if (
      ![alpha_edition, gladius_edition].includes(edition) &&
      (gold_potions < qty * 5 || legendary_potions < qty * 5)
    ) {
      if (
        !confirm(
          SM.Translator.t('open_packs.potion_warning', {
            _: `You don't have enough potions for all of your cards. Are you sure you want to continue?`,
          }),
        )
      )
        return;
    ...
    SM.BroadcastCustomJson(
      ...
      (result) => {
        ...
        if (!result || result.error) {
          ...
          if (pack_balance) pack_balance.balance -= qty;
          SM.ShowOpenPacks(pending);
          return;
        }
        if (result && result.trx_info && result.trx_info.result) {
          ...
          } else alert('An unknown error occurred opening the packs.');
          SM.ShowOpenPacks();
        } else alert('An unknown error occurred opening the packs.');
        SM.ShowOpenPacks();
      },
  ...
  ShowUnrevealedPacks: async function (trx_id) {
    SM.ShowLoading();
    var result = await SM.ApiAsync('/transactions/lookup', {
      trx_id: trx_id,
    });
    SM.HideLoading();
    console.log(result);
    if (!result || result.error) {
      alert('An unknown error occurred showing this pack opening.');
      return;
    }
    if (result && result.trx_info && result.trx_info.result) {
      var edition = tryParse(result.trx_info.data).edition;
      var trx_data = tryParse(result.trx_info.result);
      if (trx_data && trx_data.cards) {
        var unrevealed = SM.GetUnrevealedPacks();
        if (
        ...
        if (result.trx_info.type === 'open_all') {
          var groups = _.groupBy(
            trx_data.cards,
            (c) => c.card_detail_id + '#' + c.gold,
          );
  ...
  SubmitPayment: function (
    ...
    if (payment_method == 'vessel') {
      ...
      if (SM.GetBalance('DEC') < amount) {
        alert(
          `You do not have enough DEC to complete this purchase. If you have DEC on Steem Engine or Tron you must first transfer them into the game to use them for purchases.`,
        );
        return;
      }
      SM.ShowLoading();
      let p = new Promise((resolve) =>
        SM.TransferToken(
          ...
          (r) => {
            if (!r || r.error) resolve(r);
            else SM._payment_callback = resolve;
          },
        ),
      );
  ...
  PaypalComplete: function (purchase_id, tx) {
    if (!localStorage.getItem('username')) {
      ...
      if (new_account) Messenger.Connect(new_account, null, true);
    }
    SM.ShowLoading();
    return new Promise((resolve, reject) => {
      SM.Api(
  ...
  OnPurchaseComplete: async function (purchase) {
    if (purchase.type == 'starter_pack') {
      SM.HideDialog(true, function () {
        ...
        if (SM.Player.use_proxy)
          ...
          setTimeout(
            () => SM.ShowDialog('starter_set', null, null, null, 'static'),
            1e3,
          );
        loadScript('https://platform.twitter.com/oct.js', () => {
      ...
      purchase.type == 'booster_pack' ||
      purchase.type == 'cl_presale_pack' ||
      purchase.type == 'chaos_pack'
    ) {
      SM.HideDialog(true, function () {
        SM.ShowOpenPacks();
        window.scrollTo(0, 0);
        if (purchase.data) {
          ...
          if (purchase_data.piggericks_code)
            SM.ShowDialog(
              '/misc/promotions/piggericks_code',
              {
    ...
    gtag('event', 'purchase', {
      event_label: `purchase_${purchase.type}`,
      transaction_id: purchase.trx_id || purchase.id,
      affiliation: 'Splinterlands Website - Shop',
      value: parseFloat(purchase.amount_usd),
      currency: 'USD',
      ...
      items: [
        {
          ...
          name: purchase.type,
          price: purchase.qty
            ? parseFloat(purchase.amount_usd) / purchase.qty
            : parseFloat(purchase.amount_usd) / purchase.quantity,
          quantity: purchase.qty || purchase.quantity,
        },
      ],
    });
  },
  MarketPurchase: function (market_ids, price, amount, currency, callback) {
    var id = SM.settings.test_mode
      ? SM.settings.prefix + 'sm_market_purchase'
      : 'sm_market_purchase';
    if (!['CREDITS', 'DEC'].includes(currency))
      alert(
        'Error: Invalid currency specified. Only DEC and CREDITS may be used for market purchases.',
      );
    SM.ShowLoading();
    SM.MarketItemStillAvailable(
      market_ids,
      (still_available, not_available, err) => {
        if (err) {
          alert(err);
          callback();
          return;
        ...
        if (not_available.length > 0) {
          const not_available_card_list = not_available
            .map((m) => m.cards[0].uid)
            .join(', ');
          const msg = `Sorry!  The following cards are no longer available: ${not_available_card_list} \n\nPlease unselect these cards and try again.`;
          alert(msg);
          SM.HideLoading();
          if (callback) callback();
          return;
  ...
  MarketItemStillAvailable: function (market_ids, callback) {
    SM.Api('/market/status?ids=' + market_ids, null, function (data) {
      if (!data || !!data.error) {
        const err =
          'Error loading market status, please try your transaction again';
        if (callback) callback([], [], err);
        return;
      }
      let still_available = data.filter((d) => d.status === 0);
      let not_available = data.filter((d) => !still_available.includes(d));
      if (not_available.length > 0) {
        SM.Post('/market/record_market_miss');
      }
      if (callback) callback(still_available, not_available, null);
    });
  },
  ValidateMarketListing: function (market_id, card_detail_id, uid, callback) {
    const queryString = `market_id=${market_id}&card_detail_id=${card_detail_id}&uid=${uid}`;
    SM.Api(`/market/validateListing?${queryString}`, null, function (data) {
      if (!data || !!data.error) {
        const err =
          'Error validating marketlist, please try your transaction again';
        if (callback) callback(false);
        return;
      }
      if (callback) callback(data.isValid);
    });
  },
  LoadSettings: function (callback) {
    const params = {
      version: SM.settings.version || null,
      config_version: SM.settings.config_version || null,
    };
    SM.Api('/settings', params, function (response) {
      if (!response || !response.version) {
        setTimeout(function () {
          SM.Api('/settings', params, handleResponse);
        }, 2 * 1e3);
      } else {
    ...
    function handleResponse(response) {
      if (!response || !response.version) {
        ...
        if (callback) callback(SM.settings);
        return;
      } else if (SM.settings.maintenance_mode && !response.maintenance_mode) {
        if (confirm("We're ready to go! Please click ok to refresh the page."))
      ...
      if (Config.version) {
        ...
        if (settings_version > config_version) {
          if (
            confirm(
              'New updates have been pushed, please click ok to refresh the page in your browser to get the latest changes.',
            )
          )
            window.location.reload();
        }
      }
      SM.settings = Object.assign(SM.settings || {}, response);
      if (!SM.current_brawl_cycle) {
        SM.current_brawl_cycle = SM.settings.brawl_cycle;
      } else if (
        (SM.settings.brawl_cycle.id !== SM.current_brawl_cycle.id &&
          SM.settings.brawl_cycle.status === 1) ||
        (SM.settings.brawl_cycle.id === SM.current_brawl_cycle.id &&
          SM.settings.brawl_cycle.status === 1 &&
          SM.current_brawl_cycle.status === 0)
      ) {
        SM.current_brawl_cycle = SM.settings.brawl_cycle;
        if (SM.Player) {
          SM.Api('/players/refresh_guild', {}, (data) => {
            if (data && !data.error) {
              ...
              if (
                ...
                sendNotification(
                  'Get Ready to Brawl!',
                  'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
                  'Fray selection is open for the next Brawl! Go fill one to help your guild.',
      ...
      if (
        SM.settings.rpc_nodes &&
        Array.isArray(SM.settings.rpc_nodes) &&
        SM.settings.rpc_nodes.length > 0
      ) {
        Config.rpc_nodes = SM.settings.rpc_nodes.filter((n) =>
          n.startsWith('https://'),
        );
        let rpc_index = 0;
  ...
  CombineCards: function (card_detail_id, gold, card_ids, callback) {
    ...
    SM.BroadcastCustomJson(
      ...
      (result) => {
        ...
        } else if (callback) callback(details);
      },
    );
  },
  ...
  ReturnCardsFromExternalBlockchain: function (
    blockchainName,
    externalTokenIds,
  ) {
    return new Promise(async (resolve, reject) => {
      ...
      if (blockchainName === 'ethereum') {
        if (window.web3) {
          try {
            ...
            const contract = new window.web3.eth.Contract(
              JSON.parse(SM.settings.ethereum.contracts.cards.abi.result),
              SM.settings.ethereum.contracts.cards.address,
            );
            externalTokenIds.forEach((id) => {
              contract.methods
                .lockCard(id, SM.Player.name)
                .send({
                  from: address,
                })
        ...
        loadScript(
          Config.asset_location + 'scripts/libraries/waxjs/0.0.14/waxjs.js',
          async () => {
      ...
      if (blockchainName.toLowerCase() === 'ethereum') {
        if (window.web3) {
          try {
            let address = await WalletUtils.web3connect();
            const contract = new window.web3.eth.Contract(
              JSON.parse(SM.settings.ethereum.contracts.crystals.abi.result),
              SM.settings.ethereum.contracts.crystals.address,
            );
            contract.methods
              .lock(SM.Player.name, qty * 1e3)
              .send({
                from: address,
              })
              .on('confirmation', (confirmationNumber, receipt) =>
  ...
  TrxLookup: function (
    ...
    if (SM._transactions[trx_id]) {
      if (SM._transactions[trx_id].status == 'complete') {
        if (callback) callback(SM._transactions[trx_id].data);
    ...
    if (timeout > 0) {
      SM._transactions[trx_id].timeout = setTimeout(() => {
        if (
          SM._transactions[trx_id] &&
          SM._transactions[trx_id].status != 'complete'
        ) {
          if (!is_api_tx) {
            alert(
              'It appears that there has been a delay in processing this transaction from the blockchain. Please be patient and do not submit the transaction again or it will be processed twice. For pack openings, you will receive a notification in-game when your packs are ready.',
            );
            SM.HideLoading();
          }
          delete SM._transactions[trx_id];
          if (callback && !is_api_tx) callback(null);
        }
      }, timeout * 1e3);
    }
  },
  ...
  CardDetailString: function (card) {
    ...
    var splinter =
      card.color == 'Red'
        ...
        : 'Dragon';
    var str =
      '<img class="stats-type-img" src="https://d36mxiodymuqjm.cloudfront.net/website/icons/icon-rarity-' +
      rarity.toLowerCase() +
      '.svg" data-toggle="tooltip" data-placement="bottom" title="' +
      rarity +
      '"/>';
    str +=
      '<img class="stats-type-img" src="https://d36mxiodymuqjm.cloudfront.net/website/icons/icon-element-' +
      splinter.toLowerCase() +
      '.svg" data-toggle="tooltip" data-placement="bottom" title="' +
      splinter +
      '"/>';
    str +=
      '<img class="stats-type-img" src="https://d36mxiodymuqjm.cloudfront.net/website/icons/icon-type-' +
      ...
      function (result) {
        if (
          ...
          if (
            result &&
            result.error &&
            result.error.indexOf('Please refresh the page to resume') >= 0
          )
            window.location.reload();
          else {
            SM.HideDialog();
  ...
  ShowEnemyFound: function (battle_queue) {
    ...
    SM.Api(
      ...
      function (response) {
        let settings =
          typeof battle_queue.settings === 'string'
            ? tryParse(battle_queue.settings)
            : battle_queue.settings;
  ...
  SubmitTeam: async function (
    ...
    const is_swiss = extra_data?.format === 'swiss';
    const data = {
      trx_id: queue_trx,
      team_hash: team_hash,
      summoner: summoner,
          ...
          if (is_swiss) {
            if (SM._currentBattle.opponent_team_hash) {
              ...
              var notificationText =
                extra_data.battles_remaining > 1
                  ? 'Your team has taken the field! Check back later for the results; you may move on to your next battle.'
                  : 'All teams have entered the field of battle. Will you attain ultimate victory or be cast aside on the dust heap of tournament history? Time will soon tell.';
              new Noty({
                type: 'success',
                theme: 'sm',
                timeout: 3e4,
  ...
  RequestDelegation: function (id, title, data, callback) {
    ...
    SM.Api('/players/delegation', null, function (result) {
      if (result.success) {
        setTimeout(
          () => SM.BroadcastCustomJson(id, title, data, callback),
          1e3,
        );
      } else {
        ...
        if (result.error === undefined || result.error === null)
          alert(
            "Oops, it looks like you don't have enough Resource Credits to transact on the Hive blockchain.  We should be delegating more resources to your account now.  Please try this transaction again, and contact Support if this persists.",
          );
        else
          alert(
            "Oops, it looks like you don't have enough Resource Credits to transact on the Hive blockchain. Please contact Support for more information, if you have questions about this error! Error: " +
              result.error,
          );
        if (callback) callback(result);
      }
  ...
  ShowBattleScreen: function (battle_result) {
    if (battle_result) {
      ...
      if (!battle_result.inactive) battle_result.inactive = [];
    ...
    SM.Api(
      ...
      function (result) {
        ...
        if (JSON.parse(result.details).type == 'Surrender') {
          if (result.settings) result.settings = JSON.parse(result.settings);
          if (result.dec_info) result.dec_info = JSON.parse(result.dec_info);
          ...
          if (
            result.match_type === 'Tournament' &&
            result.tournament &&
            result.tournament.sub_format === 'brawl' &&
            (result.tournament.status === 1 || force_record_watched) &&
            SM.Player &&
            (SM.Player.name === result.player_1 ||
              SM.Player.name === result.player_2)
          ) {
  ...
  LoadCurrentWatched(player_name, tournament_id) {
    SM.current_watched = localStorage.getItem(
      `watched_brawl_battles::${player_name}`,
    );
    if (SM.current_watched) {
      SM.current_watched = tryParse(SM.current_watched);
      if (
        SM.current_watched &&
        SM.current_watched.tournament_id !== tournament_id
      ) {
  ...
  MarkAsWatched(id) {
    ...
    if (battle_record < 0) {
      ...
      localStorage.setItem(
        `watched_brawl_battles::${SM.current_watched.player}`,
        JSON.stringify(SM.current_watched),
      );
    }
  },
  ExitBattlePopup() {
  ...
  GetImageUrl(card, is_battle) {
    ...
    if (is_battle) folder_url = `cards_battle_${editions[card.edition]}`;
    return (
      `${Config.card_image_url}/${folder_url}/` +
      (card.skin ? card.skin + '/' : '') +
      encodeURIComponent(details.name).replace(/'/g, '%27') +
      (card.gold ? '_gold' : '') +
      (details.tier >= 7 ? '.jpg' : '.png')
    );
  },
  GetAvatarData(player) {
    const league_group = player
      ? SM.settings.leagues[player.league || 0].group
      : 'Novice';
    return {
      frame: `https://d36mxiodymuqjm.cloudfront.net/website/icons/avatars/avatar-frame_${league_group.toLowerCase()}.png`,
      icon: `https://d36mxiodymuqjm.cloudfront.net/website/icons/avatars/avatar_${
        player && player.avatar_id ? player.avatar_id : 0
      }.png`,
    };
  },
  ...
  GetMaxMonsterLevel(
    ...
    let rating_level =
      final_monster_edition === 6
        ? Math.min(SM.settings.leagues[league].level, gladiator_cap)
        : SM.settings.leagues[league].level;
    ...
    summoner_level = Math.min(
      summoner_level,
      Math.max(Math.round((summoner_max / 4) * rating_level), 1),
    );
    return Math.max(
      Math.round((monster_max / summoner_max) * summoner_level),
      1,
    );
  },
  GetRuleset(ruleset) {
  ...
  BroadcastCustomJson: async function (
    id,
    title,
    data,
    callback,
    ...
    let active_auth =
      SM.Player.require_active_auth &&
      SM.settings.active_auth_ops.includes(id.slice(id.indexOf('sm_') + 3));
    data.app = 'splinterlands/' + SM.settings.version;
    data.n = generatePassword(10);
    if (SM.settings.test_mode) data.app = SM.settings.prefix + data.app;
    let bcast_url =
    ...
    let is_api_tx = SM.settings.api_ops.includes(op_name);
    if (SM.Player.use_proxy) {
      if (is_api_tx) {
        jQuery
          .post(
            `${Config.battle_api_url || Config.api_url}/battle/battle_tx`,
            {
      ...
      jQuery.post(
        ...
        (response) => {
          if (response && response.id)
            SM.TrxLookup(response.id, false, null, callback, 10, supressErrors);
          else
            alert(
              `Error sending transaction: ${
                response ? response.error : 'Unknown error'
  ...
  BroadcastCustomJsonLocal: function (
    id,
    title,
    data,
    callback,
    ...
    if (SM._use_keychain || (active_auth && window.hive_keychain)) {
        ...
        function (response) {
          if (response.success) {
            ...
            if (response.error == 'user_cancel')
              ...
              response.error &&
              JSON.stringify(response.error).indexOf(
              ...
              rpc_node =
                Config.rpc_nodes[++SM._rpc_index % Config.rpc_nodes.length];
              steem.api.setOptions({
                transport: 'http',
                uri: rpc_node,
                url: rpc_node,
              ...
              alert(
                `There was an error publishing this transaction to the Hive blockchain. Please check to see if it went through or try again in a few minutes. Error: ${
      ...
      steem.broadcast.customJson(
        ...
        function (err, result) {
          if (result && !err) {
            SM.TrxLookup(result.id, false, null, callback, 10, supressErrors);
          } else {
            if (
              err &&
              JSON.stringify(err).indexOf('Please wait to transact') >= 0
            ) {
              SM.RequestDelegation(id, title, data, callback, retries);
              return;
            } else if (retries > 0) {
  ...
  GetTournamentMessages(tid, callback) {
    SM.Api(
      ...
      (r) => {
        if (!r.error && r.length > 0 && callback) callback(r);
      },
    );
  },
  ShowChallenge(msg) {
  ...
  ShowNoty(id, type, data, timer) {
    if (!id) id = generatePassword(10);
    if (data) data.noty_id = id;
    ...
    if (timer) {
      ...
      async function updateNotifTimer(id, block) {
        if (SM._noties[id].closed) return;
  ...
  ViewChallenge(data, msg_id) {
    if (msg_id && SM._noties[msg_id].closed) return;
    SM.Api(
      ...
      function (result) {
        if (
          (result && result.id) ||
          (SM._activeDialog != null && SM._activeDialogName == 'view_challenge')
        ) {
          new Noty({
            type: 'alert',
            theme: 'sm',
          ...
          SM.Api(
            ...
            (r) => {
              if (r && r.status == 0) {
                if (
                  SM._activeDialog != null &&
                  SM._activeDialogName == 'view_challenge'
                ) {
                  new Noty({
                    type: 'alert',
                    theme: 'sm',
                  ...
                  if (msg_id && SM._noties[msg_id]) SM._noties[msg_id].close();
  ...
  AcceptChallenge(id, opponent, settings, msg_id) {
    ...
    SM.BroadcastCustomJson(
      'sm_accept_challenge',
      'Steem Monsters Accept Challenge',
      {
        id: id,
      },
      function (result) {
        ...
        if (
          ...
          alert(
            'There was an error accepting the challenge - Error: ' +
              result.error,
          );
          SM.in_battle = false;
        }
        if (msg_id) SM.MarkAsRead(msg_id);
      },
      1,
      true,
    );
  ...
  DeclineChallenge(id, msg_id) {
    SM.BroadcastCustomJson(
      ...
      'Steem Monsters Decline Challenge',
      {
        id: id,
      },
      function (result) {
  ...
  GetCurrentBlockNum() {
    return (
      Math.max(Math.floor((Date.now() - SM.settings.timestamp) / 3e3), 0) +
      SM.settings.last_block
    );
  },
  CanPlayCard(card) {
    ...
    if (
      ...
      !card.last_used_date ||
      (card.delegated_to
        ? card.delegated_to == card.last_used_player
        : card.player == card.last_used_player)
    )
      return true;
    var cooldown_start = new Date(
      SM.settings.timestamp - SM.settings.transfer_cooldown_blocks * 3e3,
    );
    return (
      new Date(card.last_transferred_date) <= cooldown_start ||
      new Date(card.last_used_date) <= cooldown_start
    );
  },
  CooldownSeconds(card, is_market) {
    if (is_market) {
      if (!card.last_used_date) return 0;
      let cooldown_start = new Date(
        SM.settings.timestamp - SM.settings.transfer_cooldown_blocks * 3e3,
      );
      let last_used_date = new Date(card.last_used_date);
      if (last_used_date > cooldown_start) {
        var seconds_since_last_used =
          (SM.settings.timestamp - last_used_date.getTime()) / 1e3;
        return (
          SM.settings.transfer_cooldown_blocks * 3 - seconds_since_last_used
        );
      }
    } else {
  ...
  UpdateViewedBattles(battle_id) {
    if (!SM._viewedBattles) {
      ...
      if (!SM._viewedBattles) SM._viewedBattles = [];
    }
    if (SM._viewedBattles.indexOf(battle_id) >= 0) return;
    SM._viewedBattles.push(battle_id);
    localStorage.setItem(
      'sm_battles',
      JSON.stringify(SM._viewedBattles.slice(-10)),
    );
  },
  ...
  ShowTournaments() {
    ...
    SM.Api('/tournaments/in_progress', {}, (ip) => {
      ...
      SM.Api('/tournaments/upcoming', {}, (r) => {
        SM.ShowHomeView(
          'tournaments',
          ip.concat(r.filter((t) => !t.password_pub_key)),
        );
      });
    });
  },
  ShowTournament(id) {
    SM.HighlightMenuItem('tournaments');
    if (!id && current_tournament) id = current_tournament.id;
    SM.Api(
      '/tournaments/find',
      {
        id: id,
  ...
  CalculatePlayerTournamentPower(account_name, allowed_cards, collection) {
    var isEditionRestricted =
      ...
      allowed_cards.editions.length < SM.settings.num_editions;
    var player_tournament_cp = 0;
    if (
      allowed_cards.foil === 'all' &&
      allowed_cards.type === 'all' &&
      !isEditionRestricted &&
      SM.Player &&
      account_name === SM.Player.name
    ) {
      ...
      for (var card_type = 0; card_type < collection.length; card_type++) {
        ...
        if (
          ...
          (allowed_cards.type === 'no_legendaries' ||
            (allowed_cards.type === 'no_legendary_summoners' &&
              card_details.type === 'Summoner'))
        ) {
          continue;
        }
        ...
        if (owned && owned.length > 0) {
          for (var index = 0; index < owned.length; index++) {
            ...
            if (
              (card.market_id && card.market_listing_type == 'SELL') ||
              card.uid.startsWith('starter-') ||
              (card.delegated_to && card.delegated_to !== account_name) ||
              (allowed_cards.foil === 'gold_only' && !card.gold)
            ) {
              continue;
            }
            if (
              !isEditionRestricted ||
              (isEditionRestricted &&
                allowed_cards.editions.includes(card.edition))
            ) {
              player_tournament_cp += calculateCP(card);
            }
          }
        }
      }
  ...
  QualifiedTournamentFilter: function (player, collection, tournament) {
    ...
    if (spsp_min > 0) {
      if (SM.GetBalance('SPSP', player) < spsp_min) {
        return false;
      }
    }
    let [fee, token] = entry_fee.split(' ');
    if (token === 'DEC' || token === 'SPS') {
      if (alternate_fee && alternate_fee !== 'none') {
        const { league, season_max_league } = player;
        const { min_league, max_league } = alternate_fee;
        if (
          league >= min_league &&
          league <= max_league &&
          season_max_league >= min_league &&
          season_max_league <= max_league
        ) {
          [fee] = alternate_fee.value.split(' ');
        }
      }
      fee = parseFloat(fee);
      if (SM.GetBalance(token, player) < fee) {
        return false;
      }
    }
    ...
    const hasAnyCardRestriction =
      gold_only ||
      cp_min > 0 ||
      (isEditionRestricted &&
        !allowed_cards.editions.find((e) =>
          SM.settings.starter_editions.includes(e),
        ));
    if (hasAnyCardRestriction) {
      if (cp_min > 0) {
        var player_tournament_cp = SM.CalculatePlayerTournamentPower(
          player.name,
          allowed_cards,
          collection,
        );
      ...
      const summoners = collection.filter((c) => c.type == 'Summoner');
      const monsters = collection.filter((c) => c.type == 'Monster');
      let allowed_summoners, allowed_monsters;
      if (gold_only) {
        allowed_summoners = summoners.map((c) =>
          c.owned
            ? c.owned.filter(
                (o) =>
                  o.gold &&
                  (!isEditionRestricted ||
                    (isEditionRestricted &&
                      allowed_cards.editions.includes(o.edition))),
              ).length
            : 0,
        );
        allowed_monsters = monsters.map((c) =>
          c.owned
            ? c.owned.filter(
                (o) =>
                  o.gold &&
        ...
        allowed_summoners = summoners.map((c) =>
          c.owned
            ? c.owned.filter((o) => allowed_cards.editions.includes(o.edition))
                .length
            : 0,
        );
        allowed_monsters = monsters.map((c) =>
  ...
  EnterTournament: async function (
    id,
    format,
    fee,
    allowed_cards_str,
    ...
    if (
      prize_currencies &&
      prize_currencies.find((c) => {
        var supported = SM.settings.supported_currencies.find(
          (s) => s.currency == c,
        );
    ...
    SM.CheckTournamentCards(
      allowed_cards_str,
      cp_min,
      'tournament',
      format,
      null,
      () => SM.SubmitEnterTournament(id, fee, signed_pw),
    );
  },
  CheckTournamentCards(
    allowed_cards_str,
    cp_min,
    event_type,
    event_format,
    ...
    var isEditionRestricted =
      allowed_cards.editions.length > 0 &&
      allowed_cards.editions.length < SM.settings.num_editions;
    var { editionNames, editionText } = constructTournamentAllowedEditionsText(
      allowed_cards.editions,
    );
    if (
      allowed_cards.foil === 'gold_only' ||
      cp_min > 0 ||
      (isEditionRestricted &&
        !allowed_cards.editions.find((e) =>
          SM.settings.starter_editions.includes(e),
        ...
        (collection) => {
          if (cp_min > 0) {
            ...
            if (player_tournament_cp < cp_min) {
              showError(
                SM.Translator.t(
                  'components.tournaments.status_pre.min_cp_not_met',
                  {
          ...
          if (allowed_cards.foil === 'gold_only') {
            allowed_summoners = collection
              .filter((c) => c.type == 'Summoner')
              .map((c) =>
                c.owned
                  ? c.owned.filter(
                      (o) =>
            ...
            if (allowed_summoners == 0 || allowed_monsters < 6) {
              let alertMsg = player
                ? `This is a Gold Card only ${event_type} and it doesn't look like @${player} has enough Gold Cards to participate (need at least 6 Monsters and 1 Summoner).`
                : `This is a Gold Card only ${event_type} and it doesn't look like you have enough Gold Cards in your collection (need at least 6 Monsters and 1 Summoner). You can get more by purchasing booster packs or buy them directly from the market if you want to participate in these events in the future.`;
              alert(alertMsg);
            } else if (allowed_monsters < 20) {
              let confirmMsg = player
                ? `This is a Gold Card only ${event_type} and @${player} may not have enough Gold Cards to compete effectively. Are you sure you want to continue?`
                : `This is a Gold Card only ${event_type}. Please make sure you have enough Gold Cards to participate. You can see what Gold Cards you have by clicking the "Gold Only" button on the collection screen. Are you sure you want to continue?`;
            ...
            allowed_monsters = collection
              ...
              .filter((c) => c > 0).length;
            if (allowed_summoners == 0 || allowed_monsters < 6) {
              let chaos_text = allowed_cards.editions.includes(7)
                ? '(or for Chaos Legion by buying more packs from the shop) '
                : '';
              let alertMsg = player
                ? `This ${event_type} is limited to: ${editionText}. It doesn't look like @${player} has enough such cards to participate (need at least 6 Monsters and 1 Summoner).`
                : `This ${event_type} is limited to: ${editionText}. It doesn't look like you have enough such cards in your collection (need at least 6 Monsters and 1 Summoner). You can get more by purchasing them from other players on the market ${chaos_text}if you want to participate in these events in the future.`;
              alert(alertMsg);
            } else if (allowed_monsters < 20) {
              let confirmMsg = player
                ? `This ${event_type} is limited to: ${editionText}. It looks like @${player} may not have enough such cards to compete effectively. Are you sure you want to continue?`
                : `This ${event_type} is limited to: ${editionText}. Please make sure you have enough such cards to participate. You can see what cards you have by clicking on a particular card on the collection screen and then choosing the edition icon in the top right of the card details popup. Are you sure you want to continue?`;
              if (confirm(confirmMsg)) {
                callback();
                return;
              }
  ...
  SubmitEnterTournament: async function (id, fee, signed_pw) {
    ...
    if (fee && getCurrency(fee) != 'DEC' && getCurrency(fee) != 'SPS') {
      ...
      if (SM.settings.test_mode) tx_id = SM.settings.prefix + tx_id;
      var currency = SM.settings.supported_currencies.find(
        (c) => c.currency == getCurrency(fee),
      );
      ...
      if (currency.type == 'hive') {
        ...
        ssc_he
          ...
          .then((response) => {
            if (
              !response ||
              !response.balance ||
              parseFloat(response.balance) < parseFloat(fee)
            ) {
              SM.HideLoading();
              return alert(
                `You do not have enough ${currency.currency} tokens in your Hive Engine account to enter this tournament. Please go to hive-engine.com to pick some up!`,
              );
            }
            SM.HiveEngineTransfer(
              SM.settings.account,
              currency.currency,
              parseFloat(fee).toFixed(3),
              memo,
        ...
        SM.ECApi(
          ...
          async (r) => {
            if (r && !r.error)
              try {
                let tron_tx = await tronWeb.trx.sendTransaction(
                  r.wallet_address,
                  tronWeb.toSun(parseFloat(fee)),
                );
                if (tron_tx && tron_tx.code) throw tron_tx.code;
              } catch (err) {
                SM.HideLoading();
      ...
      if (
        fee &&
        !confirm(
          `This tournament has an entry fee of ${fee} which will be deducted from your balance. Do you want to continue?`,
        )
      ) {
        SM.HideLoading();
        return;
      ...
      if (parseFloat(fee) > 0) {
        ...
        if (bat_settings) {
          try {
              ...
              (result) => {
                if (!result || result.error) {
                  ...
                  if (result && result.err_code === 2) {
                    if (
                      confirm(
                        'This event requires you to hold ' +
                          bat_settings.bat +
                          " BAT in your Ethereum or BSC wallet, but you don't have an Ethereum or BSC wallet linked to the game. Would you like to link your Ethereum or BSC wallet now?",
                      )
                    ) {
                      SM.ShowDialog('link_wallet', {
                        curTab: 'ethereum',
                    ...
                    showError(
                      'This event requires you to hold ' +
                        bat_settings.bat +
                        ' BAT in your Ethereum or BSC wallet. You have ' +
                        result.balance +
                        ' BAT; please add more BAT to your wallet and then try again.',
                    );
                  } else {
                    showError(
                      'There was an error checking your BAT token balance. Please try again later.',
                    );
                  }
                }
              },
            ...
            SM.HideLoading();
            showError(
              `There was an error entering this tournament: ${err.message}`,
            );
          }
        } else {
          SM.BroadcastCustomJson(
            ...
            (result) => {
              ...
              if (result && result.error) {
                showError(
                  `There was an error entering this tournament: ${result.error}`,
                );
              }
            },
          );
  ...
  LeaveTournament(id) {
    if (
      confirm(
        'Are you sure you want to leave this tournament? Your entry fees will be refunded.',
      )
    ) {
      SM.ShowLoading();
      SM.BroadcastCustomJson(
  ...
  CancelTournament(id) {
    if (
      confirm(
        'Are you sure you want to cancel this tournament? You will lose the tournament creation fee but receive a refund for prizes held in escrow.',
      )
    ) {
      SM.ShowLoading();
      SM.BroadcastCustomJson(
  ...
  TournamentPayment: async function (tournament_id, payment) {
    ...
    if (SM.settings.test_mode) tx_id = SM.settings.prefix + tx_id;
    var currency = SM.settings.supported_currencies.find(
      (c) => c.currency == getCurrency(payment),
    );
    if (currency.type == 'hive') {
      if (window.hive_keychain) {
        hive_keychain.requestTransfer(
        ...
        async (r) => {
          if (r && !r.error)
            try {
              let tron_tx = await tronWeb.trx.sendTransaction(
                r.wallet_address,
                tronWeb.toSun(parseFloat(payment)),
              );
              if (tron_tx && tron_tx.code) throw tron_tx.code;
            } catch (err) {
              SM.HideLoading();
  ...
  EngineTransfer: function (chain, to, token, quantity, memo, callback) {
    ...
    let chain_id =
      chain == 'hive'
        ? SM.settings.ssc.hive_chain_id
        : SM.settings.ssc.chain_id;
    if (
      (chain == 'hive' && window.hive_keychain) ||
      (chain == 'steem' && window.steem_keychain)
    ) {
      let keychain = chain == 'hive' ? hive_keychain : steem_keychain;
      if (chain === 'steem') {
        SM.ApiAsync('/players/wallets')
          .then((wallets) => {
            const steemWallet = wallets.find(
              (wallet) => wallet.type == 'steem',
            );
            if (steemWallet) {
              return steemWallet.address;
            } else {
              const switchToHiveDate = new Date(
      ...
      if (chain != 'hive') {
        alert(
          'The SteemConnect service is no longer available. Please go to steem-engine.com and transfer tokens to the "steemmonsters" account to send them into the game.',
        );
    ...
    SM.Api(
      ...
      (result) => {
        if (result.trx_info) {
          ...
          setTimeout(
            () => SM.CheckSETransaction(trx_id, callback, retries + 1),
            5e3,
          );
        else if (callback) callback(result);
      },
        ...
        callbacks: {
          onClick: function () {
            SM.ShowDialog(
              'open_mystery',
              messages.map((m) =>
                Object.assign(JSON.parse(m.data), {
                  msg_id: m.id,
                }),
              ),
            );
  ...
  ShowDailyUpdate: function () {
    SM.Api('/players/daily_updates', null, (r) => {
      if (r && !r.error && r.enabled) SM.ShowDialog('daily_update', r);
    });
  },
    ...
    return Object.assign(
      {
        ...
        expiration: new Date(
          new Date(SM.settings.chain_props.time + 'Z').getTime() + 600 * 1e3,
        ),
      },
      tx,
    );
  ...
  async SignTx(tx, use_active) {
    return new Promise(async (resolve, reject) => {
      try {
        if (!tx.expiration) tx = SM.PrepareTx(tx);
        let signed_tx = null;
        if (SM._use_keychain || (use_active && window.hive_keychain)) {
          let response = await new Promise((resolve) =>
            hive_keychain.requestSignTx(
              SM.Player.name,
              tx,
              use_active ? 'Active' : 'Posting',
              resolve,
            ),
          );
          if (response && response.success) signed_tx = response.result;
  ...
  AdvanceLeaderboard: function (notify, callback) {
    if (
      confirm(
        SM.Translator.t('battle_history.adv_confirm', {
          _: `Ready to Advance? This action will result in advancing to the next League's Leaderboard to earn greater Rewards! However, you will also be battling against more challenging opponents. \n\nYou will NOT be able to return to a previous Leaderboard until next Season. \n\nIf you ever fall below the Rating or Collection Power Requirements for your new League, you will no longer qualify for that League's Leaderboard rewards (until you meet that League's Requirements).`,
        }),
      )
    ) {
  ...
  ShowVerificationStatus() {
    SM.Api('/players/id_verification_status', {}, (result) => {
      let code = null;
      if (result && result.status === 'CLEAR') code = 'verification_clear';
  ...
  async GetShopCardInfo(shop_config_name) {
    return await SM.ApiAsync(`/shop/card/${shop_config_name}`);
  },
};
$(() => {
    ...
    callbacks: {
      onTemplate: function () {
        if (this.options.type === 'challenge') {
          ...
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/swiss_battle_result',
            this.options.data,
          );
        } else if (this.options.type === 'swiss_in_progress') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/swiss_in_progress',
            this.options.data,
          );
        } else if (this.options.type === 'swiss_round_started') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/swiss_round_started',
            this.options.data,
          );
        } else if (this.options.type === 'swiss_battle_flee') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/swiss_battle_flee',
            this.options.data,
          );
        } else if (this.options.type === 'tournament_sitngo_capacity') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/tournament_sitngo_capacity',
            this.options.data,
          );
        } else if (this.options.type === 'brawl_in_progress') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/brawl_in_progress',
            this.options.data,
          );
        } else if (this.options.type === 'brawl_start') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/brawl_start',
            this.options.data,
          );
        } else if (this.options.type === 'brawl_fray_status') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/brawl_fray_status',
            this.options.data,
          );
        } else if (this.options.type === 'brawl_only_guild') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/brawl_only_guild',
            this.options.data,
          );
        } else if (this.options.type === 'pack_reveal') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/pack_reveal',
            this.options.data,
          );
        } else if (this.options.type === 'airdrop_unclaimed') {
          this.barDom.innerHTML = SM.ShowComponent(
            '/notifications/airdrop_unclaimed',
            this.options.data,
          );
        }
      },
...
Messenger = {
  ...
  Connect(player, token, new_account) {
    if (
      Messenger.ws &&
      Messenger.ws.readyState == 1 &&
      Messenger.player == player
    )
      return;
    Messenger.player = player;
    if (!Messenger.session_id) Messenger.session_id = generatePassword(10);
    ...
    if (Messenger.ping_interval) clearInterval(Messenger.ping_interval);
  ...
  OnMessage(m) {
    ...
    if (message && message.status == 'authenticated') SM.HideLoading();
  ...
  match_found(data) {
    if (
      SM.in_battle &&
      SM.CurrentView.view != 'create_team2' &&
      data.status !== 6
    ) {
  ...
  opponent_submit_team(data) {
    if (
      SM.in_battle &&
      (!SM._currentBattle ||
        (![6, 7].includes(SM._currentBattle.status) &&
          ![6, 7].includes(data.status)))
    ) {
      SM._currentBattle = data;
    }
  },
  transaction_complete(data) {
    ...
    if (data.trx_info.type === 'market_purchase') {
      let result = JSON.parse(data.trx_info.result);
      let total_usd = result.total_usd;
      gtag('event', 'purchase', {
        transaction_id: data.trx_info.id,
        affiliation: 'Splinterlands Purchase - Market',
        value: parseFloat(
          (total_usd * (SM.settings.market_fee / 1e4)).toFixed(2),
        ),
        currency: 'USD',
        purchaseType: 'market_purchase_fee',
        label: 'market_purchase_fee',
        page_path:
          localStorage.getItem('splinterlands:page_path') ||
          `/${window.location.search}`,
        items: [
          {
            id: 'market_purchase_fee',
            name: 'Market Purchase Fee',
            price: parseFloat(
              (total_usd * (SM.settings.market_fee / 1e4)).toFixed(2),
            ),
            quantity: result.num_cards,
          },
        ],
      });
    }
    if (data.trx_info.type === 'market_rent') {
      let result = JSON.parse(data.trx_info.result);
      $.get('https://prices.splinterlands.com/prices', null, (r) => {
        gtag('event', 'purchase', {
          transaction_id: data.trx_info.id,
          affiliation: 'Splinterlands Website - Market',
          value: parseFloat((r.dec * result.total_fees_dec).toFixed(2)),
          currency: 'USD',
          purchaseType: 'market_rental_fee',
          label: 'market_rental_fee',
          page_path:
            localStorage.getItem('splinterlands:page_path') ||
            `/${window.location.search}`,
          items: [
            {
              id: 'market_rental_fee',
              name: 'Market Rental Fee',
              price: parseFloat((r.dec * result.total_fees_dec).toFixed(2)),
              quantity: result.num_cards,
            },
          ],
        });
    ...
    if (
      ['open_pack', 'open_all'].includes(data.trx_info.type) &&
      timed_out &&
      !data.error
    ) {
      console.log(`Pack opening transaction timed out! Tx ID: [${id}]`);
      var unrevealed = SM.GetUnrevealedPacks();
      unrevealed.push(id);
      localStorage.setItem('sl_unrevealed_pack_openings', unrevealed);
      SM.ShowNoty(null, 'pack_reveal', {
        id: id,
      });
    }
  },
  purchase_complete(data) {
    if (SM._payment_callback) SM._payment_callback();
  ...
  market_purchase_failed(data) {
    ...
    new Noty({
      ...
      timeout: 3e4,
      text:
        'Market purchase error: ' +
        data.error +
        '. Payments have been refunded to your account.',
    }).show();
  },
  challenge(data) {
  ...
  swiss_battle_flee(data) {
    ...
    SM.ShowNoty(
      data.tournament_id + '-' + data.opponent,
      'swiss_battle_flee',
      data,
      true,
    );
    if (
      SM.Player.guild &&
      data.tournament_id === SM.Player.guild.tournament_id
    ) {
      sendNotification(
        'Brawl Battle Won!',
        'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
        '@' +
          data.opponent +
          ' has fled from your overwhelming might. Click here to see your progress!',
        () => SM.Guilds.ShowGuild(SM.Player.guild.id, 'brawl'),
      );
    } else {
      sendNotification(
        ...
        '@' +
          data.opponent +
          ' has fled from your overwhelming might. Click here to see your progress!',
        () => SM.ShowTournament(data.tournament_id),
      );
    }
  },
  swiss_battle_result(data) {
    ...
    if (
      SM.in_battle &&
      SM._currentBattle &&
      SM._currentBattle.id === data.battle_id
    ) {
      return;
    }
    SM.ShowNoty(
    ...
    if (
      ...
      sendNotification(
        'Brawl Battle Started!',
        'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
        '@' +
          data.opponent +
          ' is fighting your team. Click here to see your progress!',
        () => SM.Guilds.ShowGuild(SM.Player.guild.id, 'brawl'),
      );
    } else {
      sendNotification(
        'Anytime Battle Started!',
        'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
        '@' +
          data.opponent +
          ' is fighting your team. Click here to see your progress!',
    ...
    if (data.sub_format === 'brawl') {
      ...
      if (SM.Player.guild) {
        ...
        sendNotification(
          'Brawl Started!',
          'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
          "Your guild's next Brawl has started. Click here to play!",
          () => SM.Guilds.ShowGuild(SM.Player.guild.id, 'brawl', 'show-intro'),
        );
      }
    } else {
      ...
      sendNotification(
        'Tournament Started!',
        'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
        'The ' + data.name + ' tournament has started. Click here to play!',
        () => SM.ShowTournament(data.id),
      );
    }
  },
  ...
  tournament_checkin(data) {
    ...
    sendNotification(
      ...
      'The ' +
        data.name +
        ' tournament is starting soon. Click here to check in now!',
      () => SM.ShowTournament(data.id),
    );
  },
  tournament_sitngo_capacity(data) {
    ...
    SM.ShowNoty(
      ...
      'tournament_sitngo_capacity',
      data,
      true,
    );
    if (data.percent < 99) {
      sendNotification(
        'Sit N Go ' + data.percent + '% full!',
        'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
        'The ' +
          data.name +
          ' event has ' +
          ...
          ' of ' +
          data.max_entrants +
          ' players entered. Be ready to start soon!',
        () => SM.ShowTournament(data.id),
      );
    } else {
      sendNotification(
        'Sit N Go almost full!',
        'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
        'The ' +
          data.name +
          ' event only needs 1 more player to join before starting. Get ready to rumble!',
        () => SM.ShowTournament(data.id),
      );
    }
  },
  ...
  received_gifts(data) {
    ...
    if (
      SM.settings.card_holding_accounts.some(
        (acc) => data.from === acc.accountName,
      )
    ) {
      if (data.type == 'card') {
        ...
        if (SM.CurrentView.view === 'card_details') {
          SM.LoadCollection(SM.Player.name, true, function (collection) {
            const { card_detail_id: id, gold, edition } = data.cards[0];
  ...
  async promo_code(data) {
    ...
    if (SM.Player) {
      if (SM.Player.starter_pack_purchase) {
        alert(
          `This promo code is for a Summoner's Spellbook which has already been purchased for this account.`,
        );
        return;
      }
      let purchase = await SM.StartPurchase(
        'starter_pack',
        1,
        'PROMO',
        null,
      ...
      if (!purchase || purchase.error) {
        alert(
          'There was an error completing your purchase: ' + purchase
            ? purchase.error
            : 'Unknown error.',
        );
        return;
  ...
  guild_test(data) {
    GLD.socket(data);
    if (data.currency == 'DEC') setTimeout(() => SM.LoadBalances(), 2e3);
  },
  dec_transfer(data) {
    new Noty({
      type: 'success',
      theme: 'sm',
      timeout: 3e4,
      text: `You have received ${data.amount} ${data.token || 'DEC'} from: ${
        data.from
      }`,
    }).show();
    SM.LoadBalances();
    if (
      SM._activeDialogName == 'process_payment' &&
      data.from == '@token-converter'
    )
      SM.HideDialog();
  },
  mystery_reward(data) {
  ...
  brawl_fray_update(data) {
    if (
      $(`.brawl-body[brawl="${data.guild_id}::${data.tournament_id}"]`).length >
      0
    ) {
  ...
  brawl_fray_status(data) {
    ...
    if (SM.Player.guild) {
      ...
      let title_text = null;
      if (data.type === 'assign') {
        msg_text = `@${data.source_player} assigned you to Fray ${
          data.new_fray_index + 1
        } in the next Brawl!`;
        title_text = 'Fray Assigned';
      } else if (data.type === 'move') {
        msg_text = `@${data.source_player} moved you from Fray ${
          data.old_fray_index + 1
        } to Fray ${data.new_fray_index + 1}!`;
        title_text = 'Fray Assignment Changed';
      } else if (data.type === 'remove') {
        msg_text = `@${data.source_player} removed you from Fray ${
          data.old_fray_index + 1
        }!`;
  ...
  brawl_only_guild(data) {
    ...
    if (SM.Player.guild) {
      ...
      sendNotification(
        'Brawl Canceled',
        'https://d36mxiodymuqjm.cloudfront.net/website/ui_elements/img_challenge-sword.png',
        "Your guild's Brawl has been canceled due to a lack of opposing guilds!",
        () => SM.Guilds.ShowGuild(SM.Player.guild.id, 'brawl'),
      );
    }
  },
  guild_chat(data) {
    if ($('#chat_container').length > 0) {
      $(`#chat_container[room=${data.room_id}]`).append(
        SM.ShowComponent('chat/tavern_chat_msg', data),
      );
      ...
      $(`#brawl_chat_container[room=${data.room_id}]`).append(
        SM.ShowComponent('chat/brawl_chat_msg', data),
      );
    ...
    if (
      ($('#chat_container').length == 0 &&
        $('#brawl_chat_container').length == 0) ||
      document.hidden
    )
      sendGuildChatNotification(
        data.player == '$system' ? 'SYSTEM' : `@${data.player}`,
        data.message,
        data.room_id,
      );
  },
  ...
  potion_purchase(data) {
    if (SM._potion_purchase_callback) SM._potion_purchase_callback();
  },
  rating_update(data) {
    if (SM.Player) {
  ...
  balance_update(data) {
    if (!SM.Player) return;
    if (data.player != SM.Player.name) return;
    if (!SM.Player.balances) SM.Player.balances = [];
    ...
    else balance.balance = parseFloat(data.balance_end);
...
SM.Guilds = (function () {
  ...
  function ShowGuilds(force_list) {
    ...
    else SM.Guilds.ShowGuild(SM.Player.guild.id);
  }
  function ShowGuild(id, tab, extra_info) {
    SM.HighlightMenuItem('guilds');
    if (
      !extra_info ||
      (extra_info === 'show-intro' &&
        SM.Player &&
        SM.Player.settings.skip_brawl_anim)
    ) {
      extra_info = 'none';
    }
    SM.Api(
        ...
        SM.Api(
          ...
          (r) => {
            if (r && !r.error) {
              let is_entitled =
                r.current_member &&
                r.current_member.status == 'active' &&
                r.current_member.rank >= 3
                  ? true
                  : false;
              let building = tryParse(r.buildings)['barracks'];
              let banished_cards =
                building && building.away_blocked_cards
                  ? building.away_blocked_cards
                  : [];
              let num_tactics =
                building && building.perks ? building.perks.length : 0;
              let banish_level =
                num_tactics >= 6 ? 2 : num_tactics >= 3 ? 1 : 0;
...
class BattleTracker {
  constructor(jsonBattleResults, player1, player2, settings) {
    ...
    this.monstersCurState = keyBy('uid', [
      ...jsonBattleResults.team1.monsters,
      ...jsonBattleResults.team2.monsters,
    ]);
  ...
  get brawlPerks() {
    if (
      this.Results.is_brawl &&
      this.Results.perks &&
      !Array.isArray(this.Results.perks)
    ) {
  ...
  get numberOfActions() {
    return this.Results.rounds[this.roundIndex]
      ? this.Results.rounds[this.roundIndex].actions.length
      : 1;
  }
  ...
  nextAction() {
    ...
    this.actionIndex++;
    if (
      this.actionIndex == this.Results.rounds[this.roundIndex].actions.length
    ) {
      this.actionIndex = 0;
      this.roundIndex++;
    }
  ...
  getFutureActions(typeFilter, targetFilter) {
    if (!this.Results.rounds[this.roundIndex]) return [];
    let futureActions = this.Results.rounds[this.roundIndex].actions.slice(
      this.actionIndex,
      this.numberOfActions,
    );
    return futureActions.filter(
      (a) =>
        (!typeFilter || a.type == typeFilter) &&
        (!targetFilter || a.target == targetFilter),
    );
  }
  ...
  getMonsterById(uid) {
    return this.Results.team1.monsters
      .concat(this.Results.team2.monsters)
      .find((m) => m.uid === uid);
  }
  ...
  peekNextAction() {
    ...
    if (
      this.actionIndex + 1 ==
        this.Results.rounds[this.roundIndex].actions.length &&
      this.roundIndex + 1 == this.Results.rounds.length
    ) {
      return {
        BattleEnd: true,
      };
    ...
    if (
      ...
      action =
        this.Results.rounds[this.roundIndex].actions[this.actionIndex + 1];
    }
    return action;
  }
  ...
  restoreHistory(roundNumber) {
    ...
    if (this.roundIndex == 0) {
      ...
      $('.action_max').text(
        this.numberOfActions + this.numberOfPrebattleActions,
      );
    } else {
...
class SMBEffects {
  ...
  playRandomHitEffect() {
    if (this.spriteIndex == 21) this.spriteIndex = 1;
    const sprite = new Spriteling(
      {
        url:
          SM.settings.battles.asset_url +
          'hit_effects/' +
          this.spriteIndex +
          '.png',
        cols: 4,
  ...
  playSmoke(targetContainer) {
    ...
    $(targetContainer + ' .effect').css(
      'transform',
      'scale(1.2) rotateZ(-70deg) translateX(100px) translateY(-45px)',
    );
    sprite.play({
      run: 1,
  ...
  playResurrect(targetContainer) {
    var div = $('<div>', {
      style:
        'transform: scale(1.3) translateX(16px) translateY(-20px); z-index: 100; pointer-events: none;',
    });
  ...
  playDebuff(targetContainer) {
    var div = $('<div>', {
      style:
        'transform: scale(2.1) translateX(11px) translateY(-9px); z-index: 100; pointer-events: none;',
    });
    let sprite = new Spriteling(
      {
  ...
  playBuff(targetContainer) {
    ...
    $(targetContainer + ' .effect').css(
      'transform',
      'scale(2.1) translateX(11px) translateY(-9px)',
    );
    anime({
      targets: targetContainer + ' .card',
  ...
  playThorns(targetContainer) {
    ...
    $(targetContainer + ' .effect').css(
      'transform',
      'scale(1.5) translateX(-12px) translateY(-16.2px)',
    );
    sprite.play({
      run: 1,
      delay: 50 * SMBPlayback.iSpeed,
  ...
  playStun(targetContainer) {
    var div = $('<div>', {
      style:
        'transform: scale(1.2) translateX(7px) translateY(.8px); z-index: 100; pointer-events: none;',
    });
    let sprite = new Spriteling(
      {
  ...
  playPoison(targetContainer) {
    var div = $('<div>', {
      style:
        'transform: scale(1.2) translateX(10px) translateY(-7.2px); z-index: 100; pointer-events: none;',
    });
    let sprite = new Spriteling(
      {
  ...
  playBlast(targetContainer) {
    ...
    $(targetContainer + ' .effect').css(
      'transform',
      'scale(1.5) translateX(8px) translateY(6px)',
    );
    sprite.play({
      run: 1,
...
class SMBPortrait {
  constructor(
    ...
    $(`${targetContainer} .bio__avatar`).append(
      SM.ShowComponent('avatar', playerInfo),
    );
    $(`${targetContainer} .bio__name__pre`).text(playerInfo.title_pre || '');
    $(`${targetContainer} .bio__name__display`).text(
      playerInfo.display_name || playerInfo.name,
    );
    $(`${targetContainer} .bio__name__post`).text(playerInfo.title_post || '');
    $(`${targetContainer} .bio__guild .aspect-ratio__content`).append(
      SM.ShowComponent('guilds/crest', {
        guild_data: playerInfo.guild_data,
      }),
    );
    $(`${targetContainer} .bio__guild__name`).text(
      playerInfo.guild_name ||
        SM.Translator.t('guild.misc.no_guild', {
          _: 'No Guild',
        }),
    );
    ...
    if (summoner_stats.attack != 0) {
      $(`${targetContainer} .stats`).append(`
			<div class="battle-zoomable-obj" style="margin-bottom: 6px;" data-toggle="tooltip" data-placement="${tooltipPlacement}" data-container="body"
				...
				title="${
          summoner_stats.attack > 0
            ? SM.Translator.t('components.stats.attack_plus', {
    ...
    if (summoner_stats.ranged != 0) {
      $(`${targetContainer} .stats`).append(`
			<div class="battle-zoomable-obj" style="margin-bottom: 6px;" data-toggle="tooltip" data-placement="${tooltipPlacement}" data-container="body"
				data-template='<div class="tooltip" role="tooltip"><div class="tooltip-inner modern" style="min-width: 180px; max-width: 180px;"></div></div>'
				title="${
          summoner_stats.ranged > 0
            ? SM.Translator.t('components.stats.ranged_plus', {
    ...
    if (summoner_stats.magic != 0) {
      $(`${targetContainer} .stats`).append(`
			<div class="battle-zoomable-obj" style="margin-bottom: 6px;" data-toggle="tooltip" data-placement="${tooltipPlacement}" data-container="body"
				data-template='<div class="tooltip" role="tooltip"><div class="tooltip-inner modern" style="min-width: 180px; max-width: 180px;"></div></div>'
				title="${
          summoner_stats.magic > 0
            ? SM.Translator.t('components.stats.magic_plus', {
    ...
    if (summoner_stats.speed != 0) {
      $(`${targetContainer} .stats`).append(`
			<div class="battle-zoomable-obj" style="margin-bottom: 6px;" data-toggle="tooltip" data-placement="${tooltipPlacement}" data-container="body"
				data-template='<div class="tooltip" role="tooltip"><div class="tooltip-inner modern" style="min-width: 180px; max-width: 180px;"></div></div>'
				title="${
          summoner_stats.speed > 0
            ? SM.Translator.t('components.stats.speed_plus', {
    ...
    if (summoner_stats.armor != 0) {
      $(`${targetContainer} .stats`).append(`
			<div class="battle-zoomable-obj" style="margin-bottom: 6px;" data-toggle="tooltip" data-placement="${tooltipPlacement}" data-container="body"
				data-template='<div class="tooltip" role="tooltip"><div class="tooltip-inner modern" style="min-width: 180px; max-width: 180px;"></div></div>'
				title="${
          summoner_stats.armor > 0
            ? SM.Translator.t('components.stats.armor_plus', {
    ...
    if (summoner_stats.health != 0) {
      $(`${targetContainer} .stats`).append(`
			<div class="battle-zoomable-obj" style="margin-bottom: 6px;" data-toggle="tooltip" data-placement="${tooltipPlacement}" data-container="body"
				data-template='<div class="tooltip" role="tooltip"><div class="tooltip-inner modern" style="min-width: 180px; max-width: 180px;"></div></div>'
				title="${
          summoner_stats.health > 0
            ? SM.Translator.t('components.stats.health_plus', {
    ...
    for (let i = 0; i < summoner_stats.abilities.length; i++) {
      $(`${targetContainer} .stats`).append(`
				<img style="margin-bottom: 6px;" src="https://d36mxiodymuqjm.cloudfront.net/website/abilities/summoner/ability_${summoner_stats.abilities[
          i
        ]
          .toLowerCase()
          .replace(' ', '-')}.png"
				...
				data-template='<div class="tooltip" role="tooltip"><div class="tooltip-inner modern" style="min-width: 180px; max-width: 180px;"></div></div>'
				title="${SM.Translator.t(
          'global.summoner_abilities.' +
            summoner_stats.abilities[i].toLowerCase().replace(' ', '_'),
          {
    ...
    $(`${targetContainer} .stats`).append(`
            <div class="level">
                <div class="card-battle-level">★${summoner_stats.level}</div>
            </div>
        `);
    if (ruleset.split('|').includes('Silenced Summoners')) {
      $(`${targetContainer} .stats .battle-zoomable-obj`).addClass('disabled');
      $(`${targetContainer} .stats .battle-zoomable-obj`).attr(
        'title',
        SM.Translator.t('battle.silenced_tooltip', {
    ...
    portrait_loader.add(
      targetContainer,
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/portraits_v2/' +
        (teamInfo.summoner.skin ? teamInfo.summoner.skin + '/' : '') +
        summoner.name +
        '.png',
    );
    portrait_loader.add(
      targetContainer + '_background',
      Config.asset_location + 'scripts/battle/portrait.shader',
    );
    ...
    portrait_loader.onComplete.add((loader, resources) => {
      this.sprite = new PIXI.Sprite(
        portrait_loader.resources[targetContainer].texture,
      );
      ...
      this.filter = new PIXI.Filter(
        null,
        portrait_loader.resources[targetContainer + '_background'].data,
        uniforms,
      );
      ...
      this.portrait.ticker.add((delta) => {
        ...
        this.filter.uniforms.time += 0.1;
      });
    });
    var color =
      teamInfo.color == 'Black'
        ? 'purple'
        : teamInfo.color == 'White'
        ? '#BBB'
        : teamInfo.color == 'Gold'
        ? 'goldenrod'
        : teamInfo.color;
...
class SpineCharacter {
  constructor(targetContainer, assetName, animName, scale, x, y) {
    let divContainer = $(targetContainer);
    this.portrait = new PIXI.Application({
    ...
    divContainer.append(this.portrait.view);
    this.character_loader = new PIXI.Loader();
    this.character_loader.add(
      targetContainer,
      `https://d36mxiodymuqjm.cloudfront.net/website/${assetName}`,
    );
    this.container = new PIXI.Container();
    this.container.filterArea = this.portrait.screen;
    this.portrait.stage.addChild(this.container);
    this.animation = null;
    this.character_loader.onComplete.add((loader, resources) => {
      this.animation = new PIXI.spine.Spine(
        this.character_loader.resources[targetContainer].spineData,
      );
      this.animation.scale.set(scale);
      this.animation.x = x;
      this.animation.y = y;
      this.animation.state.setAnimation(0, animName, true);
      this.portrait.stage.addChild(this.animation);
    });
  }
  CleanUp() {
    ...
    this.character_loader = null;
    this.portrait = null;
    this.container = null;
    this.animation = null;
  }
  LoadPortraits() {
    this.character_loader.load();
  }
}
class SMBSounds {
  static MagicHit() {
    SMAudio.PlaySound(
      'snd_magicHit',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Magic_Hit.mp3',
    );
  }
  static MeleeHit() {
    SMAudio.PlaySound(
      'snd_meleeHit',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Melee_Hit.mp3',
      1,
    );
  }
  static MeleeMiss() {
    SMAudio.PlaySound(
      'snd_meleeMiss',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Melee_Miss.mp3',
      1,
    );
  }
  static RangedHit() {
    SMAudio.PlaySound(
      'snd_rangedHit',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Ranged_Hit.mp3',
      1,
    );
  }
  static RangedMiss() {
    SMAudio.PlaySound(
      'snd_rangedMiss',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Ranged_Miss.mp3',
      1,
    );
  }
  static BattleStart() {
    SMAudio.PlaySound(
      'snd_battleStart',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Stinger_BattleStart.mp3',
    );
  }
  static NextRound() {
    SMAudio.PlaySound(
      'snd_nextRound',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Stinger_NextRound.mp3',
    );
  }
  static SummonMonster() {
    SMAudio.PlaySound(
      'snd_summon',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Monster_Dispatch.mp3',
    );
  }
  static MonsterKO() {
    SMAudio.PlaySound(
      'snd_monsterKO',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Monster_KO.mp3',
    );
  }
  static FightOverInterlude() {
    SMAudio.PlaySound(
      'snd_fightOver',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Stinger_BattleOver_Victory.mp3',
    );
  }
  static PlayMainBattleTheme() {
    let songs = [
      'BurningLands',
      'CallOfWar',
      'FlightoftheElementalPhoenix',
      'ForestAwoken',
      'JournalofZintarMortalis',
      'SummonersStrife',
    ];
    let song = songs[Math.floor(Math.random() * songs.length)];
    SMAudio.PlayMusic(
      song,
      `https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/MusicLoop_${song}.mp3`,
    );
  }
  ...
  static Heal() {
    SMAudio.PlaySound(
      'snd_heal',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Heal.mp3',
    );
  }
  static Resurrect() {
    SMAudio.PlaySound(
      'snd_resurrect',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Resurrect.mp3',
    );
  }
  static Buff() {
    SMAudio.PlaySound(
      'snd_buff',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Buff.mp3',
    );
  }
  static Debuff() {
    SMAudio.PlaySound(
      'snd_debuff',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Debuff.mp3',
    );
  }
  static Thorns() {
    SMAudio.PlaySound(
      'snd_thorns',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Thorns.mp3',
    );
  }
  static Hit4() {
    SMAudio.PlaySound(
      'snd_hit4',
      'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Hit4.mp3',
    );
  }
  static Enrage() {
    setTimeout(() => {
      SMAudio.PlaySound(
        'snd_enrage',
        'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Enrage.mp3',
      );
    }, 800);
  }
  static Poison() {
    setTimeout(() => {
      SMAudio.PlaySound(
        'snd_poison',
        'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Poison.mp3',
      );
    }, 800);
  }
  static Stun() {
    setTimeout(() => {
      SMAudio.PlaySound(
        'snd_stun',
        'https://d36mxiodymuqjm.cloudfront.net/website/battle/sfx/Battle_Stun.mp3',
      );
    }, 800);
  }
}
class SMAudio {
  static PlaySound(id, sound_source, seek_skip) {
    if (SM.Player && SM.Player.settings.mute_all) return;
    if (this.sounds == null) this.sounds = [];
    if (this.sounds[id] == null) {
      this.sounds[id] = new Howl({
        src: [sound_source],
        volume: this.isSoundAndMusicMuted ? 0 : this.soundVolume,
      });
      this.sounds[id].once('load', function () {
        SMAudio.PlaySound(id, sound_source, seek_skip);
      });
    } else {
      seek_skip = seek_skip || 0;
      console.log('SMAudio - playing sound: ', id);
      this.sounds[id].seek(seek_skip).play();
    }
  }
  static StopSound(id) {
  ...
  static SetSoundVolume(volume) {
    if (this.sounds == null) return;
    if (this.isSoundAndMusicMuted) return;
    for (var audio in this.sounds) {
      this.sounds[audio].volume(volume);
    }
  }
  static PlayMusic(id, music_source) {
    if (SM.Player && SM.Player.settings.mute_all) return;
    if (this.musics == null) this.musics = [];
    if (this.musics[id] == null) {
      this.musics[id] = new Howl({
        src: [music_source],
        volume: this.isSoundAndMusicMuted ? 0 : this.musicVolume,
      });
      this.musics[id].once('load', function () {
        SMAudio.PlayMusic(id);
      });
    } else {
      console.log('SMAudio - playing music: ', id);
      this.musics[id].stop().loop(true).play();
    }
  }
  static StopMusic(id) {
    ...
    if (id) {
      ...
      for (var audio in this.musics) {
        this.musics[audio].stop();
      }
    }
  }
  static SetMusicVolume(volume) {
    ...
    if (this.isSoundAndMusicMuted) return;
    for (var audio in this.musics) {
      this.musics[audio].volume(volume);
    }
  }
  static PlayNotificationSound(id, sound_source) {
    if (SM.Player && SM.Player.settings.mute_all) return;
    if (this.notification_sounds == null) this.notification_sounds = [];
    if (this.notification_sounds[id] == null) {
      this.notification_sounds[id] = new Howl({
        src: [sound_source],
        volume: this.notificationVolume,
      });
      this.notification_sounds[id].once('load', function () {
        SMAudio.PlayNotificationSound(id, sound_source);
      });
    } else {
      console.log('SMAudio - playing notification: ', id);
      this.notification_sounds[id].play();
    }
  }
  static StopNotificationSound(id) {
    if (this.notification_sounds == null) return;
    if (id) {
      if (this.notification_sounds[id] != null) {
        this.notification_sounds[id].stop();
      }
    } else {
      for (var audio in this.notification_sounds) {
        this.notification_sounds[audio].stop();
      }
    }
  }
  static SetNotificationVolume(volume) {
    if (this.notification_sounds == null) return;
    for (var audio in this.notification_sounds) {
      this.notification_sounds[audio].volume(volume);
    }
  }
  ...
  static MuteSoundAndMusic() {
    localStorage.setItem('sl:sound_and_music_mute', 1);
    if (this.musics) {
      for (var audio in this.musics) {
        this.musics[audio].volume(0);
      }
    }
    if (this.sounds) {
      for (var audio in this.sounds) {
        this.sounds[audio].volume(0);
      }
    }
  }
  static UnmuteSoundAndMusic() {
    localStorage.setItem('sl:sound_and_music_mute', 0);
    if (this.sounds) {
      for (var audio in this.sounds) {
        this.sounds[audio].volume(this.soundVolume);
      }
    }
    if (this.musics) {
      for (var audio in this.musics) {
        this.musics[audio].volume(this.musicVolume);
      }
    }
  }
  static get isSoundAndMusicMuted() {
    return (
      (SM.Player && SM.Player.settings.mute_all) ||
      localStorage.getItem('sl:sound_and_music_mute') == 1
    );
  }
  static get soundVolume() {
    return this._getVolume('sl:sfx_volume');
  }
  static get musicVolume() {
    return this._getVolume('sl:music_volume');
  }
  static get notificationVolume() {
    return this._getVolume('sl:notifications_volume');
  }
  static _getVolume(localstorage_key) {
    let volume = parseInt(localStorage.getItem(localstorage_key));
